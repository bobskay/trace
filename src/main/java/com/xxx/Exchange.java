...
import com.xxx.biance.Api;
import com.xxx.dto.*;
import com.xxx.enums.*;
import com.xxx.response.Order;
import com.xxx.response.Ticker;
import com.xxx.utils.DateTime;
import com.xxx.utils.HttpClient;
import com.xxx.utils.UrlParamsBuilder;
import com.xxx.websocket.OrderBuilder;
import com.xxx.websocket.request.OrderDto;
        ...
        for(Account.PositionsDTO posi:this.account().getPositions()){
            if(symbol.equalsIgnoreCase(posi.getSymbol())){
                if(posi.getPositionAmt().compareTo(BigDecimal.ZERO)>0){
                    String orderId= OrderFlag.F+ DateTime.current().toString(DateTime.YEAR_TO_MILLISECOND_STRING);
    ...
    public List<Kline> kline(CandlestickInterval interval) {
        ...
        client.invoke(Api.KLINE, builder,HttpUtil::createGet,resp->{
            ...
            for(Object o:jsonArray){
                ...
                line.setTime(new DateTime(arr.getLong(0),DateTime.YEAR_TO_SECOND));
