...
public class HttpClient {
    ...
    public HttpClient() {
        this.apiSignature = new ApiSignature(Constants.SECRET_KEY);
    ...
    public <T extends ApiResponse> T invoke(Api api, UrlParamsBuilder builder,
        ...
        String resp = supply.create(requestUrl).header("X-MBX-APIKEY", Constants.API_KEY).execute().body();
        log.info("请求返回：" + resp);
    ...
    public <T extends ApiResponse> T invoke(Api api, HttpRequestSupply supply) {
        ...
        log.info("开始请求：" + requestUrl);
        String resp = supply.create(api.url).header("X-MBX-APIKEY", Constants.API_KEY).execute().body();
        log.info("请求返回：" + resp);
